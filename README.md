...
- **Small.** ~260 lines of Python, one background daemon, one menu-bar icon.
...
- **Main thread** — `rumps.App` run loop. Owns the menu bar icon.
- **pynput listener thread** — CGEventTap; fires `on_press` / `on_release` for Right Option.
- **Audio thread** — `sounddevice.InputStream` callbacks push 50 ms PCM blocks into the active recording's frame buffer.
- **Transcription worker (one per utterance)** — spawned on release; runs Parakeet, runs cleanup, pastes.
